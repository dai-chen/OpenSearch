...
import org.opensearch.querylanguages.opensearch.data.type.OpenSearchDateType;
import org.opensearch.querylanguages.opensearch.data.type.OpenSearchTextType;
import org.opensearch.querylanguages.opensearch.data.utils.Content;
import org.opensearch.querylanguages.opensearch.data.utils.ObjectContent;
import org.opensearch.querylanguages.opensearch.data.utils.OpenSearchJsonContent;
...
public class OpenSearchExprValueFactory {
    ...
    private static final Map<ExprType, BiFunction<Content, ExprType, ExprValue>> typeActionMap = new ImmutableMap.Builder<
        ExprType,
        BiFunction<Content, ExprType, ExprValue>>().put(
            OpenSearchDataType.of(OpenSearchDataType.MappingType.Integer),
            (c, dt) -> new ExprIntegerValue(c.intValue())
        )
        .put(OpenSearchDataType.of(OpenSearchDataType.MappingType.Long), (c, dt) -> new ExprLongValue(c.longValue()))
        .put(OpenSearchDataType.of(OpenSearchDataType.MappingType.Short), (c, dt) -> new ExprShortValue(c.shortValue()))
        .put(OpenSearchDataType.of(OpenSearchDataType.MappingType.Byte), (c, dt) -> new ExprByteValue(c.byteValue()))
        .put(OpenSearchDataType.of(OpenSearchDataType.MappingType.Float), (c, dt) -> new ExprFloatValue(c.floatValue()))
        .put(OpenSearchDataType.of(OpenSearchDataType.MappingType.Double), (c, dt) -> new ExprDoubleValue(c.doubleValue()))
        .put(OpenSearchTextType.of(), (c, dt) -> new OpenSearchExprTextValue(c.stringValue()))
        .put(OpenSearchDataType.of(OpenSearchDataType.MappingType.Keyword), (c, dt) -> new ExprStringValue(c.stringValue()))
        .put(OpenSearchDataType.of(OpenSearchDataType.MappingType.Boolean), (c, dt) -> ExprBooleanValue.of(c.booleanValue()))
        ...
        .put(OpenSearchDateType.of(TIMESTAMP), OpenSearchExprValueFactory::createOpenSearchDateType)
        .put(OpenSearchDateType.of(OpenSearchDataType.MappingType.Ip), (c, dt) -> new ExprIpValue(c.stringValue()))
        .put(OpenSearchDataType.of(OpenSearchDataType.MappingType.Binary), (c, dt) -> new OpenSearchExprBinaryValue(c.stringValue()))
        .put(STRING, (c, dt) -> new ExprStringValue(c.stringValue()))
        .put(INTEGER, (c, dt) -> new ExprIntegerValue(c.intValue()))
        .put(LONG, (c, dt) -> new ExprLongValue(c.longValue()))
        .put(FLOAT, (c, dt) -> new ExprFloatValue(c.floatValue()))
        .put(DOUBLE, (c, dt) -> new ExprDoubleValue(c.doubleValue()))
        .put(BOOLEAN, (c, dt) -> ExprBooleanValue.of(c.booleanValue()))
        .build();
    ...
    public ExprValue construct(String jsonString, boolean supportArrays) {
        try {
            ...
            throw new IllegalStateException(String.format(java.util.Locale.ROOT, "invalid json: %s.", jsonString), e);
        }
    }
    ...
    private ExprValue parse(Content content, String field, Optional<ExprType> fieldType, boolean supportArrays) {
        ...
        if (type.equals(OpenSearchDataType.of(OpenSearchDataType.MappingType.GeoPoint))) {
        ...
        } else if (type.equals(OpenSearchDataType.of(OpenSearchDataType.MappingType.Nested)) || content.isArray()) {
            return parseArray(content, field, type, supportArrays);
        } else if (type.equals(OpenSearchDataType.of(OpenSearchDataType.MappingType.Object)) || type == STRUCT) {
            ...
            throw new IllegalStateException(
                String.format(java.util.Locale.ROOT, "Unsupported type: %s for value: %s.", type.typeName(), content.objectValue())
            );
        }
    }
    ...
    private static ExprValue parseDateTimeString(String value, OpenSearchDateType dataType) {
        ...
        for (DateFormatter formatter : formatters) {
            try {
                ...
                switch (returnFormat) {
                    case TIME:
                        return new ExprTimeValue(zonedDateTime.withZoneSameLocal(java.time.ZoneOffset.UTC).toLocalTime());
                    case DATE:
                        return new ExprDateValue(zonedDateTime.withZoneSameLocal(java.time.ZoneOffset.UTC).toLocalDate());
                    default:
                        return new ExprTimestampValue(zonedDateTime.withZoneSameLocal(java.time.ZoneOffset.UTC).toInstant());
                }
        ...
        try {
            switch (returnFormat) {
                case TIME:
                    return new ExprTimeValue(DateFormatters.from(STRICT_HOUR_MINUTE_SECOND_FORMATTER.parse(value)).toLocalTime());
                case DATE:
                    return new ExprDateValue(DateFormatters.from(STRICT_YEAR_MONTH_DAY_FORMATTER.parse(value)).toLocalDate());
                default:
                    return new ExprTimestampValue(
                        DateFormatters.from(DateFieldMapper.getDefaultDateTimeFormatter().parse(value)).toInstant()
                    );
            }
        ...
        throw new IllegalArgumentException(
            String.format(java.util.Locale.ROOT, "Construct %s from \"%s\" failed, unsupported format.", returnFormat, value)
        );
    }
    ...
    private static ExprValue createOpenSearchDateType(Content value, ExprType type) {
        ...
        if (value.isNumber()) { // isNumber
            ...
            if (numFormatters.size() > 0 || !dt.hasFormats()) {
                long epochMillis = 0;
                if (numFormatters.contains(DateFormatter.forPattern(FormatNames.EPOCH_SECOND.getSnakeCaseName()))) {
                ...
                switch (returnFormat) {
                    case TIME:
                        return new ExprTimeValue(LocalTime.from(instant.atZone(java.time.ZoneOffset.UTC)));
                    case DATE:
                        return new ExprDateValue(LocalDate.ofInstant(instant, java.time.ZoneOffset.UTC));
                    default:
    ...
    private ExprValue parseStruct(Content content, String prefix, boolean supportArrays) {
        ...
        content.map()
            .forEachRemaining(
                entry -> populateValueRecursive(
                    result,
                    new JsonPath(entry.getKey()),
                    parse(entry.getValue(), makeField(prefix, entry.getKey()), type(makeField(prefix, entry.getKey())), supportArrays)
                )
            );
        return result;
    }
    ...
    static void populateValueRecursive(ExprTupleValue result, JsonPath path, ExprValue value) {
        if (path.getPaths().size() == 1) {
            ...
            result.tupleValue().computeIfPresent(path.getRootPath(), (key, oldValue) -> value.mergeTo(oldValue));
            result.tupleValue().putIfAbsent(path.getRootPath(), value);
        } else {
            result.tupleValue().putIfAbsent(path.getRootPath(), ExprTupleValue.empty());
            populateValueRecursive((ExprTupleValue) result.tupleValue().get(path.getRootPath()), path.getChildPath(), value);
        }
    }
    ...
    private ExprValue parseArray(Content content, String prefix, ExprType type, boolean supportArrays) {
        ...
        } else if (!(type instanceof OpenSearchDataType && ((OpenSearchDataType) type).getExprType().equals(ARRAY)) && !supportArrays) {
            return parseInnerArrayValue(content.array().next(), prefix, type, supportArrays);
        } else {
            content.array().forEachRemaining(v -> { result.add(parseInnerArrayValue(v, prefix, type, supportArrays)); });
        }

...
import static org.opensearch.querylanguages.opensearch.client.OpenSearchClient.META_CLUSTER_NAME;
...
public class OpenSearchDescribeIndexRequest implements OpenSearchSystemRequest {
            ...
            results.add(
                row(
                    ...
                    (langSpec.language() == QueryType.PPL
                        ...
                        : entry.getValue().legacyTypeName()).toLowerCase(Locale.ROOT),
                    pos++,
                    clusterName(meta)
                )
            );
    ...
    public Map<String, OpenSearchDataType> getFieldTypes() {
        ...
        Map<String, IndexMapping> indexMappings = client.getIndexMappings(getLocalIndexNames(indexName.getIndexNames()));
    ...
    public Integer getMaxResultWindow() {
        return client.getIndexMaxResultWindows(getLocalIndexNames(indexName.getIndexNames())).values().stream().min(Integer::compare).get();
    }
    ...
    private String[] getLocalIndexNames(String[] indexNames) {
        return Arrays.stream(indexNames).map(name -> name.substring(name.indexOf(":") + 1)).toArray(String[]::new);
    }

...
import org.opensearch.querylanguages.opensearch.planner.rules.EnumerableIndexScanRule;
import org.opensearch.querylanguages.opensearch.planner.rules.OpenSearchIndexRules;
import org.opensearch.querylanguages.opensearch.request.PredicateAnalyzer;
...
import org.opensearch.querylanguages.opensearch.storage.scan.context.AbstractAction;
import org.opensearch.querylanguages.opensearch.storage.scan.context.AggregationBuilderAction;
import org.opensearch.querylanguages.opensearch.storage.scan.context.FilterDigest;
import org.opensearch.querylanguages.opensearch.storage.scan.context.LimitDigest;
import org.opensearch.querylanguages.opensearch.storage.scan.context.OSRequestBuilderAction;
...
public class CalciteLogicalIndexScan extends AbstractCalciteIndexScan {
    ...
    public CalciteLogicalIndexScan copy() {
        return new CalciteLogicalIndexScan(getCluster(), traitSet, hints, table, osIndex, schema, pushDownContext.clone());
    }
    ...
    public AbstractRelNode pushDownFilter(Filter filter) {
        try {
            ...
            QueryExpression queryExpression = PredicateAnalyzer.analyzeExpression(
                filter.getCondition(),
                schema,
                fieldTypes,
                rowType,
            ...
            newScan.pushDownContext.add(
                ...
                (OSRequestBuilderAction) requestBuilder -> requestBuilder.pushDownFilter(queryExpression.builder())
            );
    ...
    private static RexNode constructCondition(List<RexNode> conditions, RexBuilder rexBuilder) {
        return conditions.size() > 1 ? rexBuilder.makeCall(SqlStdOperatorTable.AND, conditions) : conditions.get(0);
    }
    ...
    public CalciteLogicalIndexScan pushDownCollapse(Project finalOutput, String fieldName) {
        ...
        if (!ExprCoreType.numberTypes().contains(originalExprType)
            ...
            if (LOG.isDebugEnabled()) {
                LOG.debug(
                    "Cannot pushdown the dedup '{}' due to only keyword and number type are accepted, but" + " its type is {}",
                    originalFieldName,
    ...
    public CalciteLogicalIndexScan pushDownProject(List<Integer> selectedColumns) {
        ...
        CalciteLogicalIndexScan newScan = new CalciteLogicalIndexScan(
            getCluster(),
        ...
        if (pushDownContext.isAggregatePushed()) {
            ...
            List<String> projectedFields = newSchema.getFieldNames()
                .stream()
                .map(fieldName -> aliasMapping.getOrDefault(fieldName, fieldName))
                .collect(Collectors.toList());
            action = (OSRequestBuilderAction) requestBuilder -> requestBuilder.pushDownProjectStream(projectedFields.stream());
        }
    ...
    private RelTraitSet reIndexCollations(List<Integer> selectedColumns) {
        ...
        if (!Objects.isNull(relCollation) && !relCollation.getFieldCollations().isEmpty()) {
            List<RelFieldCollation> newCollations = relCollation.getFieldCollations()
                .stream()
                .filter(collation -> selectedColumns.contains(collation.getFieldIndex()))
                .map(collation -> collation.withFieldIndex(selectedColumns.indexOf(collation.getFieldIndex())))
                .collect(Collectors.toList());
    ...
    public CalciteLogicalIndexScan pushDownSortAggregateMeasure(Sort sort) {
        try {
            ...
            List<AggregationBuilder> aggregationBuilders = pushDownContext.getAggPushDownAction().getAggregationBuilder().getLeft();
            ...
            AbstractAction<?> newAction = (AggregationBuilderAction) aggAction -> aggAction.rePushDownSortAggMeasure(
            ...
            if (LOG.isDebugEnabled()) {
                LOG.debug("Cannot pushdown the sort aggregate {}", sort);
            }
        }
        return null;
    }
    ...
    public CalciteLogicalIndexScan pushDownRareTop(Project project, RareTopDigest digest) {
        try {
            ...
            AbstractAction<?> newAction = (AggregationBuilderAction) aggAction -> aggAction.rePushDownRareTop(digest);
            newScan.pushDownContext.add(PushDownType.RARE_TOP, digest, newAction);
            return newScan;
        } catch (Exception e) {
            if (LOG.isDebugEnabled()) {
                LOG.debug("Cannot pushdown {}", digest);
            }
            return null;
        }
    }
    ...
    public AbstractRelNode pushDownAggregate(Aggregate aggregate, Project project) {
        // TODO: Aggregate pushdown not yet supported in core
        return null;
    }
    ...
    public AbstractRelNode pushDownLimit(LogicalSort sort, Integer limit, Integer offset) {
        try {
            if (pushDownContext.isAggregatePushed()) {
                ...
                boolean updated = pushDownContext.getAggPushDownAction().pushDownLimitIntoBucketSize(limit + offset);
                ...
                AggregationBuilderAction action = updated
            ...
            if (LOG.isDebugEnabled()) {
                LOG.debug("Cannot pushdown limit {} with offset {}", limit, offset);
            }
        }
        return null;
    }

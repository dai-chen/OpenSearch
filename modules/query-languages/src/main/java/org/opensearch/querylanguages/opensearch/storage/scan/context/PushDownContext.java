...
public class PushDownContext extends AbstractCollection<PushDownOperation> {
    ...
    @Override
    public int size() {
        return (operationsForRequestBuilder == null ? 0 : operationsForRequestBuilder.size()) + (operationsForAgg == null
            ? 0
            : operationsForAgg.size());
    }
    ...
    public OpenSearchRequestBuilder createRequestBuilder() {
        ...
        if (operationsForRequestBuilder != null) {
            operationsForRequestBuilder.forEach(operation -> ((OSRequestBuilderAction) operation.action()).apply(newRequestBuilder));
        }

...
import static org.opensearch.querylanguages.opensearch.storage.script.aggregation.AggregationQueryBuilder.AGGREGATION_BUCKET_SIZE;
...
public class BucketAggregationBuilder {
    ...
    public MultiTermsAggregationBuilder buildMultipleTerms(List<NamedExpression> exprs) {
        MultiTermsAggregationBuilder sourceBuilder = new MultiTermsAggregationBuilder(
    ...
    public static ValuesSourceAggregationBuilder<?> buildDateHistogram(String name, String field, Integer value, SpanUnit unit) {
        ...
        switch (unit) {
            ...
            case DAY:
            case D:
                return new DateHistogramAggregationBuilder(name).field(field).fixedInterval(new DateHistogramInterval(spanValue));
            default:
                return new DateHistogramAggregationBuilder(name).field(field).calendarInterval(new DateHistogramInterval(spanValue));
        }
    }
}

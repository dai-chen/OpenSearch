...
import org.opensearch.querylanguages.opensearch.storage.scan.context.AggPushDownAction;
...
import org.opensearch.querylanguages.opensearch.storage.scan.context.PushDownOperation;
...
import static org.opensearch.querylanguages.opensearch.storage.scan.context.PushDownType.AGGREGATION;
...
public abstract class AbstractCalciteIndexScan extends TableScan {
    ...
    public RelWriter explainTerms(RelWriter pw) {
        ...
        return super.explainTerms(pw).itemIf("PushDownContext", explainString, !pushDownContext.isEmpty());
    }
    ...
    public double estimateRowCount(RelMetadataQuery mq) {
        return pushDownContext.stream().reduce(osIndex.getMaxResultWindow().doubleValue(), (rowCount, operation) -> {
            switch (operation.type()) {
                ...
                case SORT_AGG_METRICS:
                    return NumberUtil.min(rowCount, osIndex.getBucketSize().doubleValue());
                case COLLAPSE:
                    return rowCount / 10;
                case FILTER:
                case SCRIPT:
                    return NumberUtil.multiply(rowCount, RelMdUtil.guessSelectivity(((FilterDigest) operation.digest()).condition()));
                case LIMIT:
                ...
                case RARE_TOP:
                    ...
                    return groupCount == 0 ? factor : factor * rowCount * (1.0 - Math.pow(.5, groupCount));
                default:
                    return rowCount;
            }
        }, (a, b) -> null);
    ...
    public @Nullable RelOptCost computeSelfCost(RelOptPlanner planner, RelMetadataQuery mq) {
        ...
        for (PushDownOperation operation : pushDownContext) {
            switch (operation.type()) {
                ...
                case FILTER:
                    dRows = NumberUtil.multiply(dRows, RelMdUtil.guessSelectivity(((FilterDigest) operation.digest()).condition()));
                    break;
                case SCRIPT:
        ...
        */
        double estimateRowCountFactor = osIndex.getSettings().getSettingValue(CALCITE_PUSHDOWN_ROWCOUNT_ESTIMATION_FACTOR);
        return planner.getCostFactory().makeCost(dCpu * estimateRowCountFactor, 0, 0);
    }

    /** See source in Aggregate.computeSelfCost */
    ...
    protected boolean isAllCollationNamesEqualAggregators(List<String> collations) {
        Stream<LogicalAggregate> aggregates = pushDownContext.stream()
            .filter(action -> action.type() == AGGREGATION)
            .map(action -> ((LogicalAggregate) action.digest()));
        return aggregates.map(aggregate -> isAllCollationNamesEqualAggregators(aggregate, collations)).reduce(false, Boolean::logicalOr);
    }

    private boolean isAllCollationNamesEqualAggregators(LogicalAggregate aggregate, List<String> collations) {
    ...
    protected boolean isAnyCollationNameInAggregators(List<String> collations) {
        Stream<LogicalAggregate> aggregates = pushDownContext.stream()
            .filter(action -> action.type() == AGGREGATION)
            .map(action -> ((LogicalAggregate) action.digest()));
        return aggregates.map(aggregate -> isAnyCollationNameInAggregators(aggregate, collations)).reduce(false, Boolean::logicalOr);
    }
    ...
    public AbstractCalciteIndexScan pushDownSort(List<RelFieldCollation> collations) {
        try {
            ...
            if (getPushDownContext().isAggregatePushed() && isAnyCollationNameInAggregators(collationNames)) {
            ...
            if (pushDownContext.isAggregatePushed()) {
                ...
                action = (AggregationBuilderAction) aggAction -> aggAction.pushDownSortIntoAggBucket(
                    collations,
                ...
                AbstractCalciteIndexScan newScan = buildScan(
                    getCluster(),
                    traitsWithCollations,
                    hints,
                    table,
            ...
            if (LOG.isDebugEnabled()) {
                LOG.debug("Cannot pushdown the sort {}", getCollationNames(collations));
            }
        }
        return null;
    }

...
public class OpenSearchDateType extends OpenSearchDataType {
    ...
    public static final List<FormatNames> SUPPORTED_NAMED_NUMERIC_FORMATS = List.of(FormatNames.EPOCH_MILLIS, FormatNames.EPOCH_SECOND);
    ...
    private static final List<DateFormatter> OPENSEARCH_DEFAULT_FORMATTERS = Stream.of(
    ...
    public List<DateFormatter> getNumericNamedFormatters() {
        return formats.stream().filter(formatString -> {
    ...
    public List<String> getAllCustomFormats() {
        return formats.stream().filter(format -> FormatNames.forName(format) == null).map(format -> {
            try {
    ...
    public List<DateFormatter> getAllCustomFormatters() {
        return getAllCustomFormats().stream().map(DateFormatter::forPattern).collect(Collectors.toList());
    }
    ...
    public ZonedDateTime getParsedDateTime(String dateTime) {
        List<DateFormatter> dateFormatters = Stream.concat(this.getAllNamedFormatters().stream(), this.getAllCustomFormatters().stream())
            .collect(Collectors.toList());

...
public class OpenSearchRelOptUtil {
     ...
     */
    public static Pair<RexNode, RelDataType> getRemappedRexAndType(final RexNode rexNode, final RelDataType inputRowType) {
    ...
    public static Optional<Pair<Integer, Boolean>> getOrderEquivalentInputInfo(RexNode expr) {
        switch (expr.getKind()) {
            ...
            case MINUS_PREFIX:
                return getOrderEquivalentInputInfo(((RexCall) expr).getOperands().get(0)).map(
                    inputInfo -> Pair.of(inputInfo.getLeft(), !inputInfo.getRight())
                );
            case PLUS:
            case MINUS: {
                ...
                return getOrderEquivalentInputInfo(variable).map(
                    inputInfo -> Pair.of(inputInfo.getLeft(), flipped != inputInfo.getRight())
                );
            }
            case TIMES: {
                RexNode operand0 = ((RexCall) expr).getOperands().get(0);
    ...
    private static class RemapIndexBiVisitor extends RexBiVisitorImpl<Void, Pair<BitSet, List<Integer>>> {
    ...
    public RelDataType replaceDot(RelDataTypeFactory typeFactory, RelDataType rowType) {
        ...
        for (int i = 0; i < fieldList.size(); i++) {
            ...
            builder.add(new RelDataTypeFieldImpl(resolvedNames.get(i), field.getIndex(), field.getType()));
        }

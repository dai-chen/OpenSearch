...
public class OpenSearchAliasType extends OpenSearchDataType {
    ...
    public static final Set<MappingType> objectFieldTypes = Set.of(MappingType.Object, MappingType.Nested);
    ...
    public OpenSearchAliasType(String path, OpenSearchDataType type) {
        ...
        if (type instanceof OpenSearchAliasType) {
            throw new IllegalStateException(
                String.format(java.util.Locale.ROOT, "Alias field cannot refer to the path [%s] of alias type", path)
            );
        } else if (objectFieldTypes.contains(type.getMappingType())) {
            throw new IllegalStateException(
                String.format(java.util.Locale.ROOT, "Alias field cannot refer to the path [%s] of object type", path)
            );
        }
        this.path = path;

...
import org.opensearch.querylanguages.opensearch.request.PredicateAnalyzer.NamedFieldExpression;
import org.opensearch.querylanguages.opensearch.response.agg.ArgMaxMinParser;
import org.opensearch.querylanguages.opensearch.response.agg.BucketAggregationParser;
import org.opensearch.querylanguages.opensearch.response.agg.CountAsTotalHitsParser;
import org.opensearch.querylanguages.opensearch.response.agg.MetricParser;
import org.opensearch.querylanguages.opensearch.response.agg.NoBucketAggregationParser;
import org.opensearch.querylanguages.opensearch.response.agg.OpenSearchAggregationResponseParser;
import org.opensearch.querylanguages.opensearch.response.agg.SinglePercentileParser;
import org.opensearch.querylanguages.opensearch.response.agg.SingleValueParser;
import org.opensearch.querylanguages.opensearch.response.agg.StatsParser;
import org.opensearch.querylanguages.opensearch.response.agg.TopHitsParser;
import org.opensearch.querylanguages.opensearch.storage.script.aggregation.dsl.CompositeAggregationBuilder;
...
public class AggregateAnalyzer {
    ...
    public static class AggregateBuilderHelper {
        ...
        <T> T build(RexNode node, Function<String, T> fieldBuilder, Function<Script, T> scriptBuilder) {
            if (node == null) return fieldBuilder.apply(METADATA_FIELD);
                ...
                return fieldBuilder.apply(
                    new NamedFieldExpression(ref.getIndex(), rowType.getFieldNames(), fieldTypes).getReferenceForTermQuery()
                );
            } else if (node instanceof RexCall || node instanceof RexLiteral) {
                return scriptBuilder.apply((new PredicateAnalyzer.ScriptQueryExpression(node, rowType, fieldTypes, cluster)).getScript());
            }
            throw new IllegalStateException(String.format(java.util.Locale.ROOT, "Metric aggregation doesn't support RexNode %s", node));
        }
        ...
        NamedFieldExpression inferNamedField(RexNode node) {
            ...
            throw new IllegalStateException(String.format(java.util.Locale.ROOT, "Cannot infer field name from RexNode %s", node));
        }
        ...
        <T> T inferValue(RexNode node, Class<T> clazz) {
            ...
            throw new IllegalStateException(String.format(java.util.Locale.ROOT, "Cannot infer value from RexNode %s", node));
        }
    }
    ...
    public static Pair<List<AggregationBuilder>, OpenSearchAggregationResponseParser> analyze(
        ...
        try {
            ...
            Pair<Builder, List<MetricParser>> builderAndParser = processAggregateCalls(
                aggFieldNames,
            ...
            Pair<List<String>, Builder> countAggNameAndBuilderPair = removeCountAggregationBuilders(metricBuilder, countAllOnly);
            ...
            if (aggregate.getGroupSet().isEmpty()) {
                ...
                Pair<Set<Integer>, AggregationBuilder> aggPushedAndAggBuilder = createNestedAggregation(
                    groupList,
                    project,
                    subBuilder,
                    helper
                ...
                // Metric
                    ...
                    List<CompositeValuesSourceBuilder<?>> buckets = createCompositeBuckets(groupList, project, helper);
                    if (buckets.size() != groupList.size()) {
                        throw new UnsupportedOperationException(
                            "Not all the left aggregations can be converted to value sources of composite" + " aggregation"
                        );
                    }
                    AggregationBuilder compositeBuilder = AggregationBuilders.composite("composite_buckets", buckets)
                        .size(helper.bucketSize);
                    if (subBuilder != null) {
                        compositeBuilder.subAggregations(subBuilder);
                    }
                    return Pair.of(Collections.singletonList(compositeBuilder), new BucketAggregationParser(metricParsers, countAggNames));
                }
            }
        } catch (Throwable e) {
     ...
     */
    private static Pair<List<String>, Builder> removeCountAggregationBuilders(Builder metricBuilder, boolean countAllOnly) {
        List<ValueCountAggregationBuilder> countAggregatorFactories = metricBuilder.getAggregatorFactories()
            .stream()
        ...
        if (countAllOnly || supportCountFiled(countAggregatorFactories, metricBuilder)) {
            List<String> countAggNameList = countAggregatorFactories.stream()
    ...
    private static boolean supportCountFiled(List<ValueCountAggregationBuilder> countAggBuilderList, Builder metricBuilder) {
        return countAggBuilderList.size() == metricBuilder.getAggregatorFactories().size()
            && countAggBuilderList.stream().map(ValuesSourceAggregationBuilder::fieldName).distinct().count() == 1;
    }
    ...
    private static Pair<Builder, List<MetricParser>> processAggregateCalls(
        ...
        for (int i = 0; i < aggCalls.size(); i++) {
            ...
            Pair<AggregationBuilder, MetricParser> builderAndParser = createAggregationBuilderAndParser(
                aggCall,
                args,
                aggFieldName,
                helper
    ...
    private static List<RexNode> convertAggArgThroughProject(AggregateCall aggCall, Project project) {
        return project == null ? List.of() : aggCall.getArgList().stream().map(project.getProjects()::get).collect(Collectors.toList());
    }
    ...
    private static Pair<AggregationBuilder, MetricParser> createDistinctAggregation(
        ...
        switch (aggCall.getAggregation().kind) {
            case COUNT:
                return Pair.of(
                    helper.build(!args.isEmpty() ? args.get(0) : null, AggregationBuilders.cardinality(aggFieldName)),
                    new SingleValueParser(aggFieldName)
                );
            default:
                throw new AggregateAnalyzer.AggregateAnalyzerException(
                    String.format(java.util.Locale.ROOT, "unsupported distinct aggregator %s", aggCall.getAggregation())
                );
        }
    }
    ...
    private static Pair<AggregationBuilder, MetricParser> createRegularAggregation(
        ...
        switch (aggCall.getAggregation().kind) {
            case AVG:
                return Pair.of(helper.build(args.get(0), AggregationBuilders.avg(aggFieldName)), new SingleValueParser(aggFieldName));
            case SUM:
                ...
                return Pair.of(helper.build(args.get(0), AggregationBuilders.sum(aggFieldName)), new SingleValueParser(aggFieldName));
            case COUNT:
                return Pair.of(
                    helper.build(!args.isEmpty() ? args.get(0) : null, AggregationBuilders.count(aggFieldName)),
                    new SingleValueParser(aggFieldName)
                );
            case MIN: {
                ExprType fieldType = OpenSearchTypeFactory.convertRelDataTypeToExprType(args.get(0).getType());
                if (supportsMaxMinAggregation(fieldType)) {
                    return Pair.of(helper.build(args.get(0), AggregationBuilders.min(aggFieldName)), new SingleValueParser(aggFieldName));
                } else {
                    return Pair.of(
                        AggregationBuilders.topHits(aggFieldName)
                            ...
                            .size(1)
                            .from(0)
                            .sort(helper.inferNamedField(args.get(0)).getReferenceForTermQuery(), SortOrder.ASC),
            ...
            case MAX: {
                ExprType fieldType = OpenSearchTypeFactory.convertRelDataTypeToExprType(args.get(0).getType());
                if (supportsMaxMinAggregation(fieldType)) {
                    return Pair.of(helper.build(args.get(0), AggregationBuilders.max(aggFieldName)), new SingleValueParser(aggFieldName));
                } else {
                    return Pair.of(
                        AggregationBuilders.topHits(aggFieldName)
                            .fetchField(helper.inferNamedField(args.get(0)).getReferenceForTermQuery())
                            .size(1)
                            .from(0)
                            .sort(helper.inferNamedField(args.get(0)).getReferenceForTermQuery(), SortOrder.DESC),
                        new TopHitsParser(aggFieldName, true)
                    );
                }
            }
            ...
            case ARG_MAX:
                return Pair.of(
                    AggregationBuilders.topHits(aggFieldName)
                        ...
                        .size(1)
                        .from(0)
                        .sort(helper.inferNamedField(args.get(1)).getRootName(), org.opensearch.search.sort.SortOrder.DESC),
            ...
            case ARG_MIN:
                return Pair.of(
                    AggregationBuilders.topHits(aggFieldName)
                        ...
                        .size(1)
                        .from(0)
                        .sort(helper.inferNamedField(args.get(1)).getRootName(), org.opensearch.search.sort.SortOrder.ASC),
                    new ArgMaxMinParser(aggFieldName)
                );
            case OTHER_FUNCTION:
                ...
                switch (functionName) {
                    ...
                    case FIRST:
                        TopHitsAggregationBuilder firstBuilder = AggregationBuilders.topHits(aggFieldName).size(1).from(0);
                        if (!args.isEmpty()) {
                            firstBuilder.fetchField(helper.inferNamedField(args.get(0)).getReferenceForTermQuery());
                        }
                        return Pair.of(firstBuilder, new TopHitsParser(aggFieldName, true));
                    case LAST:
                        TopHitsAggregationBuilder lastBuilder = AggregationBuilders.topHits(aggFieldName)
                            .size(1)
                            .from(0)
                            .sort("_doc", org.opensearch.search.sort.SortOrder.DESC);
                        if (!args.isEmpty()) {
                            lastBuilder.fetchField(helper.inferNamedField(args.get(0)).getReferenceForTermQuery());
                        }
                        return Pair.of(lastBuilder, new TopHitsParser(aggFieldName, true));
                    case PERCENTILE_APPROX:
                        PercentilesAggregationBuilder aggBuilder = helper.build(args.get(0), AggregationBuilders.percentiles(aggFieldName))
                    ...
                    default:
                        throw new AggregateAnalyzer.AggregateAnalyzerException(
                            String.format(java.util.Locale.ROOT, "Unsupported push-down aggregator %s", aggCall.getAggregation())
                        );
                }
            default:
                throw new AggregateAnalyzer.AggregateAnalyzerException(
                    String.format(java.util.Locale.ROOT, "unsupported aggregator %s", aggCall.getAggregation())
                );
        }
    }
    ...
    private static List<CompositeValuesSourceBuilder<?>> createCompositeBuckets(
        ...
        groupList.forEach(groupIndex -> resultBuilder.add(createCompositeBucket(groupIndex, project, helper)));
    ...
    private static ValuesSourceAggregationBuilder<?> createCompositeIncompatibleAggregation(
        ...
        if (isAutoDateSpan(agg)) {
            ...
            Optional<RangeAggregationBuilder> rangeAggBuilder = CaseRangeAnalyzer.create(name, helper.rowType).analyze((RexCall) agg);
    ...
    private static AutoDateHistogramAggregationBuilder analyzeAutoDateSpan(
        ...
        return new AutoDateHistogramAggregationBuilder(name).field(helper.inferNamedField(rexInputRef).getRootName())
    ...
    private static boolean isAutoDateSpan(RexNode rex) {
        if (rex instanceof RexCall) {
            ...
            return rexCall.getKind() == SqlKind.OTHER_FUNCTION && rexCall.getOperator().equals(WIDTH_BUCKET);
        }
        return false;
    }
    ...
    private static CompositeValuesSourceBuilder<?> createTermsSourceBuilder(
        ...
        TermsValuesSourceBuilder termsBuilder = new TermsValuesSourceBuilder(bucketName).order(SortOrder.ASC);
        ...
        return withValueTypeHint(sourceBuilder, sourceBuilder::userValuetypeHint, group.getType(), group instanceof RexInputRef);
    }
    ...
    private static ValuesSourceAggregationBuilder<?> createTermsAggregationBuilder(
        ...
        TermsAggregationBuilder sourceBuilder = helper.build(
            group,
            new TermsAggregationBuilder(bucketName).size(helper.bucketSize).order(BucketOrder.key(true))
        );
        return withValueTypeHint(sourceBuilder, sourceBuilder::userValueTypeHint, group.getType(), group instanceof RexInputRef);
    }
    ...
    private static <T> T withValueTypeHint(
        ...
        ValueType valueType = ValueType.lenientParse(exprType.typeName().toLowerCase(java.util.Locale.ROOT));

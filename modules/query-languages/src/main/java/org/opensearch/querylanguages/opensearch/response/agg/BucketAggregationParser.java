...
public class BucketAggregationParser implements OpenSearchAggregationResponseParser {
    ...
    private List<Map<String, Object>> parseBucket(MultiBucketsAggregation.Bucket bucket, String name) {
        ...
        List<Map<String, Object>> results = isLeafAgg(aggregations)
    ...
    protected Optional<Map<String, Object>> extract(MultiBucketsAggregation.Bucket bucket, String name) {
        ...
        if (bucket instanceof CompositeAggregation.Bucket) {
            ...
            extracted = IntStream.range(0, keys.size()).boxed().collect(Collectors.toMap(keys::get, ((List<Object>) bucket.getKey())::get));
        } else {

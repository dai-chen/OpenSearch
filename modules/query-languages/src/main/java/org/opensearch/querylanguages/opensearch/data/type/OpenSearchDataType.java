...
public class OpenSearchDataType implements ExprType, Serializable {
    ...
    public ExprType getExprType() {
        return (exprCoreType == ExprCoreType.DATE
            ...
            || exprCoreType == ExprCoreType.UNKNOWN) ? this : exprCoreType;
    }
    ...
    static {
        EnumUtils.getEnumList(MappingType.class)
            .stream()
            .filter(t -> t != MappingType.Invalid)
            .forEach(t -> instances.put(t.toString(), OpenSearchDataType.of(t)));
        EnumUtils.getEnumList(ExprCoreType.class).forEach(t -> instances.put(t.toString(), OpenSearchDataType.of(t)));
    }
    ...
    public static Map<String, OpenSearchDataType> parseMapping(Map<String, Object> indexMapping) {
        ...
        indexMapping.forEach((k, v) -> {
            ...
            result.put(k, OpenSearchDataType.of(EnumUtils.getEnumIgnoreCase(OpenSearchDataType.MappingType.class, type), innerMap));
        });
    ...
    public static OpenSearchDataType of(MappingType mappingType, Map<String, Object> innerMap) {
        ...
        switch (mappingType) {
            ...
            case Nested:
                ...
                Map<String, OpenSearchDataType> properties = parseMapping(
                    (Map<String, Object>) innerMap.getOrDefault("properties", Map.of())
                );
            ...
            case Text:
                ...
                Map<String, OpenSearchDataType> fields = parseMapping((Map<String, Object>) innerMap.getOrDefault("fields", Map.of()));
    ...
    public boolean equals(Object o) {
        ...
        OpenSearchDataType that = (OpenSearchDataType) o;
        return mappingType == that.mappingType && exprCoreType == that.exprCoreType;
    }

    @Override
    public int hashCode() {
        return java.util.Objects.hash(mappingType, exprCoreType);
    }
    ...
    public String legacyTypeName() {
        ...
        return mappingType.toString().toUpperCase(java.util.Locale.ROOT);
    }
     ...
     * don't have nested types. See OpenSearchDataTypeTest#traverseAndFlatten() test for
     * example.
     *
        ...
        BiConsumer<Map<String, OpenSearchDataType>, String> visitLevel = new BiConsumer<>() {
            ...
            public void accept(Map<String, OpenSearchDataType> subtree, String prefix) {
                for (var entry : subtree.entrySet()) {
                    ...
                    var nextPrefix = prefix.isEmpty() ? entryKey : String.format(java.util.Locale.ROOT, "%s.%s", prefix, entryKey);

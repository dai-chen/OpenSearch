...
public class PredicateAnalyzer {
    ...
    /** Exception thrown when script expressions are not supported. */
    public static class UnsupportedScriptException extends RuntimeException {
    ...
    /**
     * Convert SQL LIKE wildcard to Lucene wildcard.
     * '%' -> '*', '_' -> '?', escape special Lucene chars.
     */
    private static String convertSqlWildcardToLucene(String sqlPattern) {
        StringBuilder lucenePattern = new StringBuilder();
        for (int i = 0; i < sqlPattern.length(); i++) {
            char c = sqlPattern.charAt(i);
            switch (c) {
                case '%':
                    lucenePattern.append('*');
                    break;
                case '_':
                    lucenePattern.append('?');
                    break;
                case '\\':
                    ...
                    break;
                default:
                    lucenePattern.append(c);
            }
        }
        return lucenePattern.toString();
    }
     ...
     */
    public static QueryBuilder analyze(RexNode expression, List<String> schema, Map<String, ExprType> fieldTypes)
    ...
    public static QueryExpression analyzeExpression(
        ...
        return analyzeExpression(expression, schema, fieldTypes, rowType, cluster, new Visitor(schema, fieldTypes, rowType, cluster));
    }
    ...
    public static QueryExpression analyzeExpression(
        ...
        try {
            ...
            try {
                // TODO: Script query support not yet available in core
    ...
    static class Visitor extends RexVisitorImpl<Expression> {
                ...
                case SPECIAL:
                    switch (call.getKind()) {
                        ...
                        case CONTAINS:
                            return binary(call);
                        default:
                            String message = String.format(java.util.Locale.ROOT, "Unsupported call: [%s]", call);
                            throw new PredicateAnalyzerException(message);
                    }
        ...
        private QueryExpression visitRelevanceFunc(RexCall call) {
            ...
            if (SINGLE_FIELD_RELEVANCE_FUNCTION_SET.contains(funcName) && ops.size() < 2) {
                throw new PredicateAnalyzerException(
                    "Single field relevance query function should at least have 2 operands (field and" + " query)"
                );
            ...
            if (SINGLE_FIELD_RELEVANCE_FUNCTION_SET.contains(funcName)) {
                List<Expression> fieldQueryOperands = visitList(
                    List.of(AliasPair.from(ops.get(0), funcName).value, AliasPair.from(ops.get(1), funcName).value)
                );
                ...
                Map<String, String> optionalArguments = parseRelevanceFunctionOptionalArguments(ops, funcName);

                return SINGLE_FIELD_RELEVANCE_FUNCTION_HANDLERS.get(funcName)
                ...
                if ("fields".equals(firstKey)) {
                    ...
                    queryLiteralOperand = ((LiteralExpression) visitList(List.of(AliasPair.from(ops.get(1), funcName).value)).get(0))
                        .stringValue();
                    ...
                    queryLiteralOperand = ((LiteralExpression) visitList(List.of(firstPair.value)).get(0)).stringValue();
                    optionalArguments = parseRelevanceFunctionOptionalArguments(ops, funcName, 1);
                } else {
                    throw new PredicateAnalyzerException(
                        format(
                            Locale.ROOT,
                            "Invalid first parameter for function [%s]: expected 'fields' or 'query', got" + " '%s'",
                            funcName,
                            firstKey
                        )
                    );
            ...
            throw new PredicateAnalyzerException(format(Locale.ROOT, "Unsupported search relevance function: [%s]", funcName));
        }
        ...
        private static final Map<String, SingleFieldRelevanceFunctionHandler> SINGLE_FIELD_RELEVANCE_FUNCTION_HANDLERS = Map.of(
            "match",
            (f, q, o) -> QueryExpression.create(f).match(q, o),
            "match_phrase",
            (f, q, o) -> QueryExpression.create(f).matchPhrase(q, o),
            "match_bool_prefix",
            (f, q, o) -> QueryExpression.create(f).matchBoolPrefix(q, o),
            "match_phrase_prefix",
            (f, q, o) -> QueryExpression.create(f).matchPhrasePrefix(q, o)
        );

        private static final Map<String, MultiFieldsRelevanceFunctionHandler> MULTI_FIELDS_RELEVANCE_FUNCTION_HANDLERS = Map.of(
            "simple_query_string",
            (c, q, o) -> QueryExpression.create(new NamedFieldExpression()).simpleQueryString(c, q, o),
            "query_string",
            (c, q, o) -> QueryExpression.create(new NamedFieldExpression()).queryString(c, q, o),
            "multi_match",
            (c, q, o) -> QueryExpression.create(new NamedFieldExpression()).multiMatch(c, q, o)
        );
        ...
        private Map<String, String> parseRelevanceFunctionOptionalArguments(List<RexNode> operands, String funcName, int startIndex) {
            ...
            for (int i = startIndex; i < operands.size(); i++) {
                ...
                if (optionalArguments.containsKey(key)) {
                    throw new PredicateAnalyzerException(
                        format(Locale.ROOT, "Parameter '%s' can only be specified once for function [%s].", key, funcName)
                    );
                }
        ...
        private QueryExpression prefix(RexCall call) {
            checkArgument(call.getKind() == SqlKind.NOT, "Expected %s got %s", SqlKind.NOT, call.getKind());
        ...
        private QueryExpression postfix(RexCall call) {
            checkArgument(call.getKind() == SqlKind.IS_TRUE || call.getKind() == SqlKind.IS_NULL || call.getKind() == SqlKind.IS_NOT_NULL);
            if (call.getOperands().size() != 1) {
                ...
                case SEARCH:
                    ...
                    switch (nullAs) {
                        ...
                        case TRUE:
                            ...
                            finalExpression = CompoundQueryExpression.or(expression, QueryExpression.create(pair.getKey()).notExists());
                            break;
                        case UNKNOWN:
        ...
        private static QueryExpression constructQueryExpressionForSearch(RexCall call, SwapResult pair) {
            if (isSearchWithComplementedPoints(call)) {
                ...
                boolean isTimeStamp = (pair.getKey() instanceof NamedFieldExpression)
                    && ((NamedFieldExpression) pair.getKey()).isTimeStampType();
                List<QueryExpression> queryExpressions = rangeSet.stream()
                    .map(
                        range -> RangeSets.isPoint(range)
                            ? QueryExpression.create(pair.getKey()).equals(sargPointValue(range.lowerEndpoint()), isTimeStamp)
        ...
        private boolean containIsEmptyFunction(RexCall call) {
            return call.getKind() == SqlKind.OR
                ...
                && call.getOperands()
                    .stream()
                    .anyMatch(o -> o.getKind() == SqlKind.OTHER && ((RexCall) o).getOperator().equals(SqlStdOperatorTable.IS_EMPTY));
        }
            ...
            switch (call.getKind()) {
                ...
                case AND:
                    if (failedCount == call.getOperands().size()) {
                        ...
                        throw new PredicateAnalyzerException("All expressions in AND failed to analyze: " + call);
                    }
        ...
        public Expression tryAnalyzeOperand(RexNode node) {
            try {
                ...
                // TODO: Script query support not yet available in core
                throw new PredicateAnalyzerException(firstFailed);
            }
        }
        ...
        private static SwapResult swap(Expression left, Expression right) {
            ...
            if (literal == null || terminal == null) {
                String message = format(Locale.ROOT, "Unexpected combination of expressions [left: %s] [right: %s]", left, right);
                throw new PredicateAnalyzerException(message);
            }
    ...
    public abstract static class QueryExpression implements Expression {
        ...
        QueryExpression exists() {
            throw new PredicateAnalyzerException("SqlOperatorImpl ['exists'] " + "cannot be applied to " + this.getClass());
        }

        QueryExpression notExists() {
            throw new PredicateAnalyzerException("SqlOperatorImpl ['notExists'] " + "cannot be applied to " + this.getClass());
        }

        QueryExpression contains(LiteralExpression literal) {
            throw new PredicateAnalyzerException("SqlOperatorImpl ['contains'] " + "cannot be applied to " + this.getClass());
        }
        ...
        QueryExpression like(LiteralExpression literal) {
            throw new PredicateAnalyzerException("SqlOperatorImpl ['like'] " + "cannot be applied to " + this.getClass());
        }

        QueryExpression notLike(LiteralExpression literal) {
            throw new PredicateAnalyzerException("SqlOperatorImpl ['notLike'] " + "cannot be applied to " + this.getClass());
        }

        QueryExpression equals(LiteralExpression literal) {
            throw new PredicateAnalyzerException("SqlOperatorImpl ['='] " + "cannot be applied to " + this.getClass());
        }
        ...
        QueryExpression notEquals(LiteralExpression literal) {
            throw new PredicateAnalyzerException("SqlOperatorImpl ['not'] " + "cannot be applied to " + this.getClass());
        }
        ...
        QueryExpression matchBoolPrefix(String query, Map<String, String> optionalArguments) {
            throw new PredicateAnalyzerException("MatchBoolPrefix " + "cannot be applied to " + this.getClass());
        }

        QueryExpression matchPhrasePrefix(String query, Map<String, String> optionalArguments) {
            throw new PredicateAnalyzerException("MatchPhrasePrefix " + "cannot be applied to " + this.getClass());
        }

        QueryExpression simpleQueryString(RexCall fieldsRexCall, String query, Map<String, String> optionalArguments) {
            throw new PredicateAnalyzerException("SimpleQueryString " + "cannot be applied to " + this.getClass());
        }
    ...
    public static class CompoundQueryExpression extends QueryExpression {
        ...
        private List<RexNode> analyzedNodes = new ArrayList<>();
        private final List<RexNode> unAnalyzableNodes = new ArrayList<>();
    ...
    static class SimpleQueryExpression extends QueryExpression {
        ...
        private String getFieldReferenceForTermQuery() {
            ...
            if (reference == null) {
                throw new PredicateAnalyzerException("Field reference for term query cannot be null for " + rel.getRootName());
            }
            return reference;
        }
        ...
        public QueryExpression like(LiteralExpression literal) {
            ...
            if (isKeywordField) {
                builder = wildcardQuery(keywordField, convertSqlWildcardToLucene(literal.stringValue())).caseInsensitive(true);
                return this;
            }
        ...
        public QueryExpression equals(LiteralExpression literal) {
            ...
            if (literal.isDateTime()) {
                builder = addFormatIfNecessary(literal, rangeQuery(getFieldReference()).gte(value).lte(value));
            } else {
        ...
        public QueryExpression notEquals(LiteralExpression literal) {
            Object value = literal.value();
            if (literal.isDateTime()) {
                builder = boolQuery().should(addFormatIfNecessary(literal, rangeQuery(getFieldReference()).gt(value)))
        ...
        public QueryExpression match(String query, Map<String, String> optionalArguments) {
            // TODO: Relevance functions not yet supported in core
            throw new UnsupportedOperationException("Relevance functions not yet supported in core");
        }
        ...
        public QueryExpression between(Range<?> range, boolean isTimeStamp) {
            Object lowerBound = range.hasLowerBound() ? convertEndpointValue(range.lowerEndpoint(), isTimeStamp) : null;
            Object upperBound = range.hasUpperBound() ? convertEndpointValue(range.upperEndpoint(), isTimeStamp) : null;
            RangeQueryBuilder rangeQueryBuilder = rangeQuery(getFieldReference());
            rangeQueryBuilder = range.lowerBoundType() == BoundType.CLOSED
                ? rangeQueryBuilder.gte(lowerBound)
                : rangeQueryBuilder.gt(lowerBound);
            rangeQueryBuilder = range.upperBoundType() == BoundType.CLOSED
    ...
    private static String timestampValueForPushDown(String value) {
        ...
        return DateFieldMapper.getDefaultDateTimeFormatter().format(exprTimestampValue.timestampValue());
    ...
    private static void checkForNestedFieldOperands(RexCall call) throws PredicateAnalyzerException {
        boolean conditionContainsNestedField = call.getOperands()
            .stream()
            .map(RexNode::getType)
        ...
        if (conditionContainsNestedField) {
            throw new PredicateAnalyzerException(
                format(Locale.ROOT, "OpenSearch DSL does not handle %s on nested fields correctly", call.getKind())
            );
        }
    }
}

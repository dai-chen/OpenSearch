...
public class CompositeAggregationBuilder {
    ...
    public List<CompositeValuesSourceBuilder<?>> build(
        ...
        ImmutableList.Builder<CompositeValuesSourceBuilder<?>> resultBuilder = new ImmutableList.Builder<>();
        for (Triple<NamedExpression, SortOrder, MissingOrder> groupPair : groupList) {
            resultBuilder.add(
                buildCompositeValuesSourceBuilder(groupPair.getLeft(), groupPair.getMiddle(), groupPair.getRight(), bucketNullable)
            );
        }
    ...
    private CompositeValuesSourceBuilder<?> buildCompositeValuesSourceBuilder(
        ...
        if (expr.getDelegated() instanceof SpanExpression) {
            ...
            if ((expr.getDelegated().type() instanceof OpenSearchDateType
                && List.of(TIMESTAMP, TIME, DATE).contains(((OpenSearchDateType) expr.getDelegated().type()).getExprCoreType()))
    ...
    public static CompositeValuesSourceBuilder<?> buildHistogram(
        ...
        switch (unit) {
            case NONE:
                HistogramValuesSourceBuilder histogramBuilder = new HistogramValuesSourceBuilder(name).field(field).interval(value);
                if (bucketNullable) {
    ...
    public static CompositeValuesSourceBuilder<?> buildDateHistogram(String name, String field, Integer value, SpanUnit unit) {
        ...
        switch (unit) {
            ...
            case DAY:
            case D:
                return new DateHistogramValuesSourceBuilder(name).field(field).fixedInterval(new DateHistogramInterval(spanValue));
            default:
                return new DateHistogramValuesSourceBuilder(name).field(field).calendarInterval(new DateHistogramInterval(spanValue));
        }
    }
}

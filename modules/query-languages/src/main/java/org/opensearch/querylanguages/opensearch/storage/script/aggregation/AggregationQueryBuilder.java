...
package org.opensearch.querylanguages.opensearch.storage.script.aggregation;
...
import org.opensearch.querylanguages.opensearch.storage.script.aggregation.dsl.MetricAggregationBuilder;
...
public class AggregationQueryBuilder extends ExpressionNodeVisitor<AggregationBuilder, Object> {
    ...
    public Pair<List<AggregationBuilder>, OpenSearchAggregationResponseParser> buildAggregationBuilder(
        ...
        final Pair<AggregatorFactories.Builder, List<MetricParser>> metrics = metricBuilder.build(namedAggregatorList);
                    ...
                    AggregationBuilders.composite(
                        ...
                        compositeBuilder.build(
                            groupByList.stream()
                                ...
                                .map(expr -> Triple.of(expr, groupSortOrder.sortOrder(expr), groupSortOrder.missingOrder(expr)))
                                .collect(Collectors.toList()),
                            bucketNullable
                        )
                    ).subAggregations(metrics.getLeft()).size(AGGREGATION_BUCKET_SIZE)
                ),
    ...
    public Map<String, OpenSearchDataType> buildTypeMapping(List<NamedAggregator> namedAggregatorList, List<NamedExpression> groupByList) {
        ...
        namedAggregatorList.forEach(agg -> builder.put(agg.getName(), OpenSearchDataType.of(agg.type())));
    ...
    public static class GroupSortOrder implements Comparator<NamedExpression> {
         ...
         */
        private static final Pair<Sort.SortOption, Integer> DEFAULT_ORDER = Pair.of(Sort.SortOption.DEFAULT_ASC, Integer.MAX_VALUE);

        /** The mapping between {@link Sort.SortOrder} and {@link SortOrder}. */
        private static final Map<Sort.SortOrder, SortOrder> SORT_MAP = new ImmutableMap.Builder<Sort.SortOrder, SortOrder>().put(
            Sort.SortOrder.ASC,
            SortOrder.ASC
        ).put(Sort.SortOrder.DESC, SortOrder.DESC).build();

        /** The mapping between {@link Sort.NullOrder} and {@link MissingOrder}. */
        private static final Map<Sort.NullOrder, MissingOrder> NULL_MAP = new ImmutableMap.Builder<Sort.NullOrder, MissingOrder>().put(
            Sort.NullOrder.NULL_FIRST,
            MissingOrder.FIRST
        ...
        public GroupSortOrder(List<Pair<Sort.SortOption, Expression>> sortList) {
            ...
            for (Pair<Sort.SortOption, Expression> sortPair : sortList) {
                map.put(((ReferenceExpression) sortPair.getRight()).getAttr(), Pair.of(sortPair.getLeft(), pos++));
            }
        }

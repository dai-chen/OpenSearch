...
final class LuceneFragmentPlanner {
    ...
    private static InputColumn docValuesColumn(List<FieldStorageInfo> storage, int ordinal) {
        ...
        ColumnKind kind = switch (info.getFieldType()) {
            case DATE -> ColumnKind.TIMESTAMP;
            case LONG -> ColumnKind.LONG;
            case KEYWORD -> ColumnKind.KEYWORD;
            case BOOLEAN -> ColumnKind.BOOLEAN;
            case FLOAT -> ColumnKind.FLOAT;
            case DOUBLE -> ColumnKind.DOUBLE;
            case BINARY -> ColumnKind.BINARY;
            case IP -> ColumnKind.IP;
            default -> null;
        };
        return kind == null ? null : new InputColumn(info.getFieldName(), kind);
    }
